            ...
            {filteredJobs.map((job, index) => (
              <React.Fragment key={job.id}>
                ...
                <div className="flex items-center justify-between mb-4 relative z-10">
                  ...
                  {job.is_urgent ? (
                    <span className="flex items-center gap-1.5 px-2 py-0.5 md:px-3 md:py-1 bg-rose-100 text-rose-600 rounded-full text-[10px] md:text-xs font-black uppercase tracking-wider border border-rose-200 shadow-sm">
                      <Zap className="w-3.5 h-3.5 fill-current" /> SOS
                    </span>
                  ) : (
                    <span className="flex items-center gap-1.5 px-2 py-0.5 md:px-3 md:py-1 bg-blue-50 text-blue-600 rounded-full text-xs md:text-sm font-extrabold border border-blue-100 shadow-sm">
                      {job.category}
                    </span>
                  )}
                ...
                <Link href={`/job/${job.id}`}>
                  <h3 className="text-base md:text-lg font-bold text-gray-900 mb-1 leading-snug group-hover:text-indigo-600 transition-colors line-clamp-2 relative z-10">
                    {job.category === "Physical" ? "🛠️ " : job.category === "Digital" ? "💻 " : job.category === "Tutoring" ? "📚 " : "✨ "}{job.title}
                  </h3>
                </Link>
                <p className="text-xs md:text-sm text-gray-600 line-clamp-2 mb-3 flex-1">
                  {job.description}
                </p>

                {/* Meta Row */}
                <div className="flex items-center gap-4 text-xs md:text-sm font-semibold text-gray-700 mb-3 pt-2 md:pt-3 border-t border-gray-100">
                  <div className="flex items-center gap-1.5 bg-gray-50 px-2 md:px-3 py-1 md:py-1.5 rounded-lg">
                ...
                <div className="flex items-center justify-between mt-auto">
                  <Link href={`/user/${job.requester_id}`} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-gray-800 to-black flex items-center justify-center text-white font-bold text-sm shadow-sm">
                  ...
                  <Link 
                    href={`/job/${job.id}`}
                    className="px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl bg-gray-100 text-gray-900 font-bold text-xs md:text-sm hover:bg-slate-200 active:scale-95 transition-all"
                  >
                    View Details
                  </Link>
                </div>
