...
function FriendsContent() {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-[#FAFAFA] font-sans max-w-4xl mx-auto w-full p-6">
      <h1 className="text-3xl font-black text-gray-900 mb-8">Connections</h1>
      
      {/* Tabs */}
      <div className="flex gap-4 mb-8 bg-gray-100 p-1.5 rounded-2xl w-fit">
        <button 
          onClick={() => setActiveTab('friends')}
          className={`px-6 py-2 rounded-xl font-bold text-sm transition-all ${activeTab === 'friends' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'}`}
        >
          My Friends
        </button>
        <button 
          onClick={() => setActiveTab('requests')}
          className={`px-6 py-2 rounded-xl font-bold text-sm transition-all ${activeTab === 'requests' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'}`}
        >
          Friend Requests
        </button>
        <button 
          onClick={() => setActiveTab('search')}
          className={`px-6 py-2 rounded-xl font-bold text-sm transition-all ${activeTab === 'search' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'}`}
        >
          Find Friends
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {friends.length === 0 ? (
            ...
            friends.map(f => {
              ...
              return (
                <div key={f.id} className="bg-white border border-gray-100 p-5 rounded-3xl shadow-sm flex items-center justify-between group hover:shadow-md transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {requests.length === 0 ? (
            ...
            requests.map(req => (
              <div key={req.id} className="bg-white border border-gray-100 p-5 rounded-3xl shadow-sm flex items-center justify-between">
                <div className="flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleReject(req.id)} className="w-10 h-10 rounded-full bg-red-50 text-red-600 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors">
        ...
        <div className="space-y-6 max-w-2xl mx-auto">
          <form onSubmit={handleSearch} className="flex gap-3">
            <div className="flex-1 bg-white border border-gray-200 rounded-2xl flex items-center px-4 focus-within:border-black focus-within:ring-2 focus-within:ring-black/5 transition-all shadow-sm">
            ...
            <button 
              ...
              className="px-8 bg-black text-white rounded-2xl font-bold hover:bg-gray-900 disabled:opacity-50 transition-all shadow-md active:scale-95"
            >
          ...
          <div className="space-y-4">
            ...
            {searchResults.map(user => (
              <div key={user.id} className="bg-white border border-gray-100 p-5 rounded-3xl shadow-sm flex items-center justify-between">
                <div className="flex items-center gap-4">
                ...
                <button 
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-gray-50 text-black font-bold text-sm rounded-xl hover:bg-gray-200 active:scale-95 transition-all"
                >
