...
export default function Navigation() {
  ...
  return (
    <>
      ...
      <nav 
        className="md:hidden fixed bottom-0 left-0 right-0 w-full bg-white/95 border-t border-gray-200/50 flex justify-around items-center h-[calc(64px+env(safe-area-inset-bottom))] z-[100] px-1 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_20px_rgba(0,0,0,0.05)]"
      >
        ...
        <a href="/explore" className={`flex flex-col items-center justify-center w-full h-full transition-colors group ${pathname.startsWith('/explore') ? 'text-black' : 'text-gray-400 hover:text-black'}`}>
          <Compass className={`w-[22px] h-[22px] group-active:scale-90 transition-transform ${pathname.startsWith('/explore') ? 'fill-black' : ''}`} />
        </a>
        
        {/* Center Action Button */}
        <a href="/create" className="flex flex-col items-center justify-center w-full h-full group relative -top-4">
        ...
        <a href="/chat" className={`flex flex-col items-center justify-center w-full h-full transition-colors group ${pathname.startsWith('/chat') ? 'text-black' : 'text-gray-400 hover:text-black'}`}>
          <MessageSquare className={`w-[22px] h-[22px] group-active:scale-90 transition-transform ${pathname.startsWith('/chat') ? 'fill-black' : ''}`} />
        </a>
        <a href="/gigs" className={`flex flex-col items-center justify-center w-full h-full transition-colors group ${pathname.startsWith('/gigs') ? 'text-black' : 'text-gray-400 hover:text-black'}`}>
          <Briefcase className={`w-[22px] h-[22px] group-active:scale-90 transition-transform ${pathname.startsWith('/gigs') ? 'fill-black' : ''}`} />
        </a>
        <a href="/profile" className={`flex flex-col items-center justify-center w-full h-full transition-colors group ${pathname.startsWith('/profile') ? 'text-black' : 'text-gray-400 hover:text-black'}`}>
